...
<%
	...
	String sMenuId = ApprUtil.setnullvalue(request.getParameter("menu"),  ApprMenuId.ID_240_NUM );//메뉴번호
...
//     if(apprreadInfo.getApprovalType().equals(ApprDocCode.APPR_NUM_6)&&apprreadInfo.getApprIngNo()==0){
    ...
    int apprSize = apprformInfo.getApprCnt();
    int helpSize = apprformInfo.getHelpCnt();
//------------------------------------------------------------------------------------------------------------------
    // 첨부 파일 경로 가져오기   
...
//도움말
var APPR_SIZE = <%= apprformInfo.getApprCnt() %>;
...
//결재 참조 문서 팝업
function goRefDocViewer(docs){
	var tmp = docs.split(":");
	var url= "/approval/apprdoc.htm?apprId=" + tmp[1] + "&menu=130&cmd=EDIT";
			...
			for ( var i=0; i < appobj.rows.length-1; i++ ) {
				for ( var j=0; j < appobj.rows[i].cells.length; j++) {
					if ( i==0 ) {
						...
						$(F_TABLE.rows[i].cells[j]).append( $(appobj.rows[i+1].cells[j]).html() );
						$(F_TABLE.rows[i].cells[j]).css("font-size","8pt");
						$(F_TABLE.rows[i].cells[j]).css("line-height","10px");
						//$(F_TABLE.rows[i].cells[j]).css("font-family","tahoma");
					}
				}
			}
...
// 			F_TopNode.parentNode.removeChild(F_TopNode);
			...
			if(R_TABLE){
				...
				R_TopNode.parentNode.removeChild(R_TopNode);
			}
		<%} %>
		...
		<td height="27"> 
			<table width="100%" border="0" cellspacing="0" cellpadding="0" height="27">
				<tr>
					...
					<td width="400" class="SubTitle" style="font-size:11pt; font-wieght:bold;"><b><!--<spring:message code="appr.form" text="전자결재 양식"/> - --><%=apprformInfo.getSubject(locale) %></b></td>
					<td valign="bottom" width="*" align="right"> 
						<a onclick="javascript:docPrint();" class="button white medium"><!-- 인쇄미리보기 Code appr.print.preview -->
...
<%if(!ApprUtil.nullCheck(apprreadInfo.getRelateValue()).equals("")){ %>

<!-- 관련문서 시작 -->
<table class=apsize style="top:-1px;" id="ApReceipt">
<tr height=30>
<td width=110 align=center bgcolor="#EDF2F5">결재문서 첨부</td>
<td style="word-break:break-all;" align=left>
	<%
	if(apprreadInfo.getApprovalType().equals(ApprDocCode.APPR_NUM_6)){
	%>
		<a href ="javascript:goRefDocViewer('<%= ApprUtil.nullCheck(apprOldInfo.getRelateValue()) %>');"><%= ApprUtil.nullCheck(apprOldInfo.getRelateText()) %></a>
	<%}else{ %>
		<a href ="javascript:goRefDocViewer('<%= ApprUtil.nullCheck(apprreadInfo.getRelateValue()) %>');"><%= ApprUtil.nullCheck(apprreadInfo.getRelateText()) %></a>
	<%} %>
</td>
</tr>
</table>
